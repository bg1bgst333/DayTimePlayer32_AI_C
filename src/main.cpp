...
{
    mciSendStringW(L"stop wav",  NULL, 0, NULL);
    mciSendStringW(L"close wav", NULL, 0, NULL);
    g_playing = false;
}
...
static void OpenFile(HWND hwnd)
{
    ...
    StopFile();
    g_wavPath = buf;
    SetDlgItemTextW(hwnd, IDC_LABEL_FILE, buf);
    UpdateButtons(hwnd);
}
...
static void PlayFile(HWND hwnd)
{
    ...
    std::wstring openCmd = L"open \"" + g_wavPath + L"\" type waveaudio alias wav";
    MCIERROR err = mciSendStringW(openCmd.c_str(), NULL, 0, NULL);
    if (err != 0) {
        wchar_t errBuf[256] = {0};
        mciGetErrorStringW(err, errBuf, 256);
        MessageBoxW(hwnd, errBuf, L"再生エラー", MB_ICONERROR);
        return;
    }

    mciSendStringW(L"play wav notify", NULL, 0, hwnd);
    g_playing = true;
    UpdateButtons(hwnd);
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_CREATE:
    {
        ...
        CreateWindowExW(0, L"STATIC", L"ファイルを選択してください",
        ...
        CreateWindowExW(0, L"BUTTON", L"再生",
            WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON | WS_DISABLED,
            100, 40, 80, 30,
            hwnd, (HMENU)IDC_BTN_PLAY, hInst, NULL);

        CreateWindowExW(0, L"BUTTON", L"停止",
            WS_CHILD | WS_VISIBLE | BS_PUSHBUTTON | WS_DISABLED,
            190, 40, 80, 30,
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE, LPSTR, int nCmdShow)
{
    ...
    wc.lpszClassName = L"WavPlayer";
    wc.hCursor       = LoadCursorW(NULL, (LPCWSTR)IDC_ARROW);
    wc.hIcon         = LoadIconW(NULL, (LPCWSTR)IDI_APPLICATION);
    RegisterClassW(&wc);

    HWND hwnd = CreateWindowExW(
        0, L"WavPlayer", L"WAV Player",
